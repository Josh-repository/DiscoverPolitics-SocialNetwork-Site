...
class Postw
{
		...
		public function submitPost($body,$user_to)
		{
			...
			//insert post
				$query=mysqli_query($this->con,"INSERT INTO postw VALUES('','$body','$added_by','$user_to','$date_added','no','no','0')");
	...
	public function loadPostsFriends($data, $limit) {
		...
		$data_query = mysqli_query($this->con, "SELECT * FROM postw WHERE deleted='no' ORDER BY id DESC");
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($added_by_obj->isClosed()) {
					...
					$str .= "<div class='status_post'onClick='javascript:toggle$id()' >
							...
							<iframe src='likesw.php?post_id=$id' scrolling='no'></iframe>

							
							</div>
							<div class='post_comment' id='toggleComment$id' style='display:none;''>
							<iframe src='commentsw.php?post_id=$id' id='comment_iframe' frameborder='0'>
							</iframe>
							</div>
							<hr>
					...
					<script>
						...
						$(document).ready(function()
						{
							...
							bootbox.confirm("Are you sure you want to delete this post?",function(result)
								{	
									$.post("delete_postw.php?post_id=<?php echo $id;?>",{result:result});
									if(result)
										location.reload();
